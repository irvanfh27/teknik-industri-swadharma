    ...
    $(document).ready( function () {
        ...
        //$('#dataTables-home-log').DataTable();
        $('#dataTables-home-list').DataTable({
            "bFilter": true,
            "paging":   false,
            //"iDisplayLength": 20,
            "order": [[ 0, "asc" ]]
    ...
    function edit_home_popup(id,judul,sejarah,visi,misi){
        $("#edit-home-id").val(id);
        $( "#edit-judul" ).val(judul);
        $( "#edit-sejarah" ).val(sejarah);
        $( "#edit-visi" ).val(visi);
        $( "#edit-misi" ).val(misi);
        $('#editHomeSubmit').attr("onclick","update_home_details("+id+")");
    }

    function deactivate_confirmation(email,id){
        $( "#home-email" ).html(email);
        $('#deactivateYesButton').attr("onclick","deactivate_submit('"+email+"',"+id+")");
    }

    function reset_confirmation(email,id){
        $( "#reset-home-email" ).html(email);
    ...
    function deactivate_submit(email,id){
        show_loading();
            $.ajax({
            url: $("#base-url").val()+"admin/deactivate_home/"+email+"/"+id,
            cache: false,
    ...
    function reset_submit(email,id){
        show_loading();
            $.ajax({
            url: $("#base-url").val()+"admin/reset_home_password/"+email+"/"+id,
            cache: false,
            success: function (result) {
                var result = $.parseJSON(result);
                if(result.status=='success'){
    ...
    function update_home_details(id){
        hideErrorMessages();
        show_loading();
        var i=0;
        ...
        if(i == 0){
            $.ajax({
                url: $("#base-url").val()+"admin/update_home_details/",
                traditional: true,
                type: "post",
                dataType: "text",
    ...
    $( "#newhomeSubmit" ).click(function() {
        ...
        if(i == 0){
            $.ajax({
                url: $("#base-url").val() + "admin/add_home",
                traditional: true,
                type: "post",
                dataType: "text",
